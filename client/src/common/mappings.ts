...
): { data: InventoryTezos[]; action: "update" | "delete" | "create" } => {
  return {
    data: invetories.map((inventory: InventoryDB) => {
      return {
        id: inventory.id,
...
export const mapAssetToTezos = (
  assets: AssetDB[],
  action: "update" | "delete" | "create"
): { data: AssetTezos[]; action: "update" | "delete" | "create" } => {
  return {
    data: assets.map((asset: AssetDB) => {
      return {
...
export const mapInvoiceToTezos = (
  invoices: InvoiceDB[],
  action: "update" | "delete" | "create"
): { data: InvoiceTezos[]; action: "update" | "delete" | "create" } => {
  return {
    data: invoices.map((invoice: InvoiceDB) => {
      return {
        id: invoice.id,
...
export const mapProductToTezos = (
  products: ProductDB[],
  action: "update" | "delete" | "create"
): {
  data: ProductTezos[];
  ...
  return {
    data: products.map((product: ProductDB) => {
      return {
        id: product.id,
...
export const mapTransactionToTezos = (
  ...
  action: "update" | "delete" | "create"
): { data: TransactionTezos[]; action: "update" | "delete" | "create" } => {
  return {
    data: transactions.map((transaction: TransactionDB) => {
      return {
